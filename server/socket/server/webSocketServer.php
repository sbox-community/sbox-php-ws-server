...
//stream_set_read_buffer($Socket, 0); // no buffering hgs 01.05.2021
                ...
                if ($Client->Handshake) {
                    ...
                    // For s&box
                    // Password checking should be before of the handshaking?
                    if ($Client->needPW) {
                        if($this->password != '')
                        {
                            ...
                            if($message != '')
                            {
                                ...
                                if(is_object($decoded) && isset($decoded->password) && strval($decoded->password) == $this->password)
                                {
                                    $pwiscorrect = true;
                                    $Client->needPW = false;
                                }
                            ...
                            if(!$pwiscorrect)
                            {
                                $this->onError($SocketID, "Password is not correct, disconnected");
                                $this->Close($Socket);
                                continue;
                            }
                        }
                ...
                $this->Clients[$SocketID]->needPW = $this->password != '';
    ...
    private function specificChecks($SocketID) {
        ...
        } else if (count($this->allowedIP) > 0) {  // s&box; why?; && $this->Clients[$SocketID]->clientType != 'websocket'
            /*
             * ***********************************************
